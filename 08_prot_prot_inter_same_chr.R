...
for(i in 1:length(gene_cis)){
  ...
  cis_gene_tss_i = pos_gene$start[which(pos_gene$gene == cis_gene_i)]
  ...
  ## genes in trans on the same chr
  gene_trans = pos_gene$gene[which(pos_gene$chr == cis_chr & 
                                     abs(pos_gene$start - cis_gene_tss_i) > 10000000)]
...
saveRDS(p_save, paste0('cis_trans_inter/07_prot_inter/prot_prot_inter/sandwich_p/cis_chr', 
                       cis_chr, '_trans_chr', cis_chr, '.rds'))

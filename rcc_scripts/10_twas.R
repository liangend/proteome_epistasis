...
sumstats_file = 'gtex/10_GWAS_coloc/GWAS_sum_stats/cc_trait/AD_Kunkle.txt.gz'
weights_pos = 'cis_trans_inter/00_ref/ukb_fusion.pos'
weights_dir = 'cis_trans_inter/06_fusion_model/'
ref_geno = paste0('gtex/11_ldsc/00_ref/1000G_EUR_Phase3_plink/1000G.EUR.QC.', chr)
out = paste0('cis_trans_inter/10_twas/AD/chr', chr, '.dat')
force_model = 'enet'
min_r2pred = 0.7
max_impute = 0.5
...
colnames(sumstat)[which(colnames(sumstat) %in% c('Tested_Allele', 'Other_Allele'))] = 
  c('A1', 'A2')
  ...
  if ( length(cur.Z) == 0 ) {
    ...
    if ( sum(cur.miss) != 0 ) {
      if ( sum(!cur.miss) == 0 ) {
        ...
        cur.FAIL = TRUE
      } else if ( mean(cur.miss) > max_impute ) {

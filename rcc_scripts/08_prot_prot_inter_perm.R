...
# expression matrix prot_ex; protein name gene_name; and sample id sample_id
...
if (cis_chr == trans_chr) {
  ...
  pred_all = list.files('cis_trans_inter/06_fusion_pred/', pattern = '_good_pred.txt')
  ...
  for(i in 1:length(gene_trans)){
    ...
    trans_pred_i = fread(paste0('cis_trans_inter/06_fusion_pred/', 
    ...
    for (j in 1:length(gene_cis)) {
      ...
      cis_pred_j = fread(paste0('cis_trans_inter/06_fusion_pred/', 
      ...
      cis_ex_j = cis_ex_j[as.character(common_id)]
      ...
      ## permute trans predicted value
      #trans_prot_i = sample(trans_prot_i)
      trans_prot_i = rnorm(length(cis_prot_j))
  ...
  saveRDS(p_save, paste0('cis_trans_inter/07_prot_inter/prot_prot_inter/perm_p2/cis_chr', 

...
for(flag_i in 1:nrow(pos_gene)){
  ...
  if(!(length(exp_gflag)==0)){
    ## extract cis regions
    tss1=max(0,pos_gene$start_hg19[flag_i]-1000000)
    tss2=pos_gene$start_hg19[flag_i]+1000000
    ...
    if(length(cis_flag)>0){
      tryCatch({
        ...
        fwrite(allmap[cis_flag,c(1,4,4,4)], 
               paste0("cis_trans_inter/03_geno_grm/", prot_i, ".txt"), col.names=F, sep = '\t')
        system(paste0(plink_dir, "plink --bfile ",genotype_dir,
		      "chr_all --exclude cis_trans_inter/03_geno_grm/",
                      prot_i,".txt --keep cis_trans_inter/01_obs_phe/", prot_i, 
                      ".txt --make-bed --range --thread-num 16 --out cis_trans_inter/03_geno_grm/", prot_i))
        ...
        system(paste0(gcta_dir, 
                      ...
                      " --qcovar cis_trans_inter/00_ref/gcta_cov_less.qcovar --out cis_trans_inter/07_h2/trans_h2_1mb/", 
                      prot_i))
      }, error=function(e){})
    }
  }

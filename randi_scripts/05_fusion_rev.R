...
tmp_dir = paste0('cis_trans_inter/05_fusion/tmp/rev',chr,'/')
...
for(i in nrow(expr_chr):1){
  ...
  if(length(flag_i)>0){
    ...
    fwrite(data.frame(chr = chr, start = tss1, end = tss2, prot = prot_i),
           paste0("cis_trans_inter/05_fusion/rev_geno/", prot_i, ".txt"), col.names = F, sep = '\t')
    system(paste0(plink_dir, " --bfile ", genotype_dir, "chr",chr,
                  ...
                  ".txt --extract range cis_trans_inter/05_fusion/rev_geno/", prot_i,
                  ".txt --make-bed --out cis_trans_inter/05_fusion/rev_geno/chr", chr))

    ## calculate cis snp weight using fusion. Set h2 = 0.2 to skip h2 estimation since it takes too long
    system(paste0('Rscript ', fusion_dir, 'FUSION.compute_weights.R --bfile cis_trans_inter/05_fusion/rev_geno/chr',

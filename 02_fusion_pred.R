...
output_dir = 'cis_trans_inter/05_fusion/pred_expr/'

pos_gene = fread(gene_pos_dir)
all_model = list.files('cis_trans_inter/05_fusion/output/')
model_done = list.files('cis_trans_inter/05_fusion/pred_expr')
model_done = sapply(strsplit(model_done, '_', fixed = T), '[', 1)
all_model = all_model[!(all_model %in% paste0(model_done, '.wgt.RDat'))]

load('/gpfs/data/xliu-lab/jinghui/ukb_ppp/ukb_ppp_prot_expr.rdata')
...
for (i in all_model) {
  ...
  pred_id = fread(paste0(expr_dir, 'pred_id_chr', chr_i, '.txt'))
  fwrite(data.frame(FID = pred_id$id,
                    IID = pred_id$id),
         paste0('cis_trans_inter/05_fusion/pred_id/chr', chr_i, '.txt'), sep = '\t')
  ...
  system(paste0(plink_dir, " --bfile ", genotype_dir, "chr", chr_i,
                "_QC --keep cis_trans_inter/05_fusion/pred_id/chr", chr_i,
                ".txt --extract range cis_trans_inter/05_fusion/cis_geno/", prot_i,
                ".txt --make-bed --out cis_trans_inter/05_fusion/cis_geno/chr", chr_i))

  ## predict prot expr using fusion weights
  load(paste0('cis_trans_inter/05_fusion/output/', i))
  weight_i = wgt.matrix
  cis_geno = read_plink(paste0('cis_trans_inter/05_fusion/cis_geno/chr', chr_i),
                        impute="avg")
  ...
  ## save the prediction and calculate the cor between obs and pred
